...
const AddAppointment = ({ onSendAppointment, lastId }) => {
  ...
  function formDataPublish() {
   ...
   return (
        ...
        {
          toggleForm &&
          <div className="border border-gray-400 border-r-2 border-b-2 border-l-2 border-light-blue-500 rounded-b-md pl-4 pr-4 pb-4">
          <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start  sm:pt-5">
            <label htmlFor="ownerName" className="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">
              Owner Name
            </label>
            <div className="border border-gray-400 mt-1 sm:mt-0 sm:col-span-2">
              <input type="text" name="ownerName" id="ownerName"
                onChange={(event) => { setFormData({ ...formData, ownerName: event.target.value }) }}
                value={formData.ownerName}
                className="max-w-lg block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:max-w-none style={{ maxWidth: '42rem' }} sm:text-sm border-gray-300 rounded-md" />
            </div>
          </div>
